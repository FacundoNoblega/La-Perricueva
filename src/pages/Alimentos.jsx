...
const Alimentos = () => {
  ...
  useEffect(() => {
    document.body.classList.add('page-alimentos');
    return () => document.body.classList.remove('page-alimentos');
  }, []);

  // ... (La lógica de filtrado robusto no cambia) ...
  const filteredProducts = alimentos
    .filter(prod => {
  ...
  if (loading) return <div className="loading-spinner">Cargando alimentos...</div>;
  ...
  return (
    <>
      ...
      <h1 style={{ color: 'white' }}>Alimentos Balanceados</h1>
      <p>La mejor nutrición para perros, gatos, aves y más.</p>

      {/* Quick Nav (no cambia) */}
      <div className="quick-nav">
        <Link to="/alimentos" className="active">Alimentos</Link>
      ...
      <div className="search-filter">
        <input className="search-input" placeholder="Buscar alimentos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      </div>
